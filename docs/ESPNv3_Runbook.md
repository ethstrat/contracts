...
- [x] 1. **Treasury/redemption/offerer multisig = `0x0cbe9bDD425a7d651e6D4FE292c8504eEa4ef26D`.** Confirmed (SO6, 2026-09-24): redemption Safe `0x0cbe9bDD425a7d651e6D4FE292c8504eEa4ef26D`.
- [ ] 2. **Reward token for the 28-day `StakedStrat` reward stream = USDS.** Founder sign-off on USDS as the yield token: **still open.**
- [x] 3. **"Stop yield" semantics.** Closed: `finalYieldAmount` = 0. Snapshot 26043909 was taken (2026-09-24), `StopEspnYield` is not needed, and a nonzero value makes Distribute refuse (freshness check). Original notes kept below.
  **Notes:** `StopEspnYield.s.sol` makes one final `increaseAssetsPerShare()` call, after which no more calls are made from this project. Three things this does **not** give you, all needing confirmation:
...
- [x] 4. **Tripwire controller.** Closed: controller `0x328aED8F7a01f45A959c187F3cb97eC508064854` (commit 7a0a6a96), recorded in `internalAddresses.json`.
- [x] 5. Exclusion list (`excludedAddresses` in `settings.json`) now holds three addresses: the Uniswap V4 PoolManager singleton (`0x000000000004444c5dc75cB358380D2e3dE08A90`), the treasury/redemption Safe (`0x0cbe9bDD425a7d651e6D4FE292c8504eEa4ef26D`, same as Assumption 1) and Seaport (`0x0000000000000068F116a894984e2DB1123eB395`). ESPN's LP is a V4 pool; at the last snapshot the PoolManager held a dust amount (~0.00036 ESPN) but is excluded regardless since any future-funded position would land there too.
- [x] 6. Disposition of ESPN the treasury reclaims from fills: hold the reclaimed ESPN until after Distribute. Any ESPN burn or `increaseAssetsPerShare` before Distribute trips the snapshot freshness check and needs a new snapshot.
...
`poolId` the `ProposeLp` log prints; (4) check the Safe's USDS balance is >= 500,000.

**LP batch recovery (SO3).** To regenerate the LP batch at a different tick spacing: set `lp.tickSpacing` in
`settings.json` from 60 to 30, delete the existing `script/deployments/1/multisig/005-earn-lp/001-0x0cbe9bDD-multisig.json`,
then rerun `ProposeLp.s.sol`. It refuses while the file exists.
...
All three Verify scripts (`verify:redemption`, `verify:migration`, `verify:lp`) derive the holders file from `SNAPSHOT_BLOCK` — it is the only env var
...
Step 4a (`004-stry-migration/Deploy.s.sol`) broadcasts from the deployer's key and writes `.staked-earn` to `deploymentAddresses.json`; it needs a live TripwireController (Assumption 4) and `.stry` with code.
...
- **Recovery.** If the broadcast fails after the deploy or `mintBatch` and EARN `owner()` != redemption Safe, the deployer calls `transferOwnership(safe)` on EARN manually, then reconciles `.stry` and `.earn-airdrop-supply` in `deploymentAddresses.json` to the chain before any re-run.

Steps 1, 4b, 5, 7, 8 (`StopEspnYield`, `ProposeLp`, `BuildOrder`, `PeriodicYield`, `Cancel`) never broadcast —
each writes a Safe batch instead (see section 6). Steps 3, 4 and 4a broadcast directly from the
deployer's own key: the two `Distribute` scripts mint and airdrop; `Deploy.s.sol` deploys sEARN. `PeriodicYield.s.sol`
...
`settings.json` ships `finalYieldAmount = "0"`, and `0` is the decision (Assumption 3): snapshot
26043909 was taken, `StopEspnYield.s.sol` (step 1) is not needed, and a nonzero value would
move ESPN NAV and make Distribute refuse.
