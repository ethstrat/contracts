...
- `campaignLookup(campaignId) == 0` — surfaces a duplicate before the batch is signed, not at execution.
...
5. **Assert registration**: `campaignLookup(id) != 0`; `campaign(id)` has `creator == safe`, `rewardToken == USDS`, `campaignType == 18`, `startTimestamp`, `duration == 604800`, `campaignData` all equal to the built params, and `amount == gross * (1e9 - fees) / 1e9` where `fees` is derived from `campaignSpecificFees(18)` / `defaultFees()` / `feeRebate(safe)` exactly as `_computeFees` does.
