...
Format: recommendation | reason | alternatives. **Signed off by the user on 2026-09-24: SO1 resolved as below; SO2–SO6 accepted as recommended.**

- SO1. Safe USDS runway (O8). **Resolved: no top-up plan needed.** PeriodicYield runs every 28 days and requires only that the Safe holds one period's amount (~33,791 USDS) at each run (`require(balanceOf(safe) >= amount)`); fund each period before running it. | Fact: 615,087.33 USDS − 500,000 LP = 115,087.33 USDS remains, enough for the first ~3.4 periods without any inflow.
...
- SO6. Assumption 1 (O6). **Confirmed by the user on 2026-09-24 against the Safe{Wallet} UI.** Recommendation was: confirm `0x0cbe9bDD425a7d651e6D4FE292c8504eEa4ef26D` as the redemption Safe. | Reason: on-chain it is a Safe 1.4.1, 1-of-1 under the main multisig, holding 615,087.33 USDS at block 26043909. Under D8, a wrong target is unrecoverable. | Alternative: none. Nothing broadcasts until it is confirmed.
