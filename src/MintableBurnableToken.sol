 ...
 */
contract MintableBurnableToken is ERC20Permit, Ownable2Step, TripwireGuard {
    ...
    constructor(
        ...
        address guardian_
    ) ERC20(name, symbol) ERC20Permit(name) Ownable(owner) TripwireGuard(controller_, guardian_) {}
     ...
     */
    function mint(address to, uint256 amount) external onlyMinter whenNotTripped {
        _mint(to, amount);
    }
     ...
     */
    function burn(uint256 amount) external whenNotTripped {
     ...
     */
    function burnFrom(address from, uint256 amount) external whenNotTripped {
