...
/// StakedStrat instance's 7-day reward stream. No approve() step: transfer, not transferFrom.
contract WeeklyYield is Script {
    function run() external virtual {
        ...
        SafeBatchLib.Tx[] memory txs = weeklyYield(safe, usds, stakedEarn, amount);
        ...
        SafeBatchLib.write(
            ...
            "Weekly STRY yield",
            "Transfers USDS to the StakedStrat contract and calls syncRewards() to start a new 7-day reward stream. Execute the transactions in the order listed.",
            txs
        );
    }
    ...
    /// syncRewards() sees totalDeposited <= totalNotifiedRewards and early-returns. Funding before
    /// anyone has staked permanently destroys the deposit.
    function weeklyYield(address safe, address usds, address stakedEarnAddr, uint256 amount)
        internal
        view
        returns (SafeBatchLib.Tx[] memory txs)
    {
        ...
        require(
            ...
            "WeeklyYield: totalStaked() == 0 -- funding now permanently destroys the deposit, see src/StakedStrat.sol syncRewards()"
        );
    ...
    /// which overwrites silently. A stale index would rewrite a previous week's batch in place
    /// with a different amount, with no error -- and a batch mid-signature-collection in the Safe
    /// UI would stop matching what the next signer diffs against. So there is no index env var and
    /// no manual counter: 001 is taken by StopEspnYield.s.sol, so start at 2 and take the first
    /// free index. Redoing a week whose batch was generated but not signed means deleting that
